...
│   ├── msid                      # Read MSID
│   ├── take-ownership --new-pw*  # MSID → SID PIN 변경 (DESTRUCTIVE, --force)
│   ├── activate                  # Locking SP 활성화 (DESTRUCTIVE, --force, auth: SID)
│   ├── setup --new-pw*           # take-ownership + activate 묶음 (DESTRUCTIVE, --force)
│   ├── revert --sp ...           # Factory reset (DESTRUCTIVE, --force)
│   └── psid-revert --psid ...    # PSID-based reset (DESTRUCTIVE, --force)
├── range
│   ├── list                      # Enumerate ranges
│   ├── setup --id --start --len  # Configure start/length (DESTRUCTIVE, --force)
│   ├── lock --read --write       # Set lock state
│   └── erase --id                # Crypto-erase (DESTRUCTIVE, --force)
├── band                          # Enterprise SSC
│   ├── list
...
│   ├── enable --id               # Enable user authority
│   ├── assign --id --range       # Assign user to range ACL
│   ├── set-pw --id --new-pw*     # Set user password
│   └── setup --id --range --new-pw*   # enable + set-pw + assign 묶음
├── mbr
...
└── eval                          # Evaluator-only
    ...
    ├── transaction --script ...  # Run JSON script in one session
    └── fault-list                # Enumerate FaultBuilder fault points (read-only)
```

> `drive setup`은 새 SID 비밀번호 하나로 take-ownership + activate 를 묶어 수행한다.
> 활성화 직후 Admin1 PIN은 SID와 동일하므로 동일 비밀번호를 `-p / --pw-env / --pw-file`로
> 전달해 `range setup`을 바로 이어갈 수 있다. SID와 Admin1 PIN을 분리하려면 take-ownership
> → activate 를 따로 호출하고 그 사이에 Admin1 PIN을 별도로 설정.
...
둘 이상 지정하면 `EC_USAGE` 반환.

### `--new-pw*` (목적지 비밀번호) — 별도 트리오

`--password*` 가 **현재 인증용** 비밀번호라면, `--new-pw*` 는 **새로 설정할 목적지** 비밀번호다. 다음 명령이 이 트리오를 사용한다:

| 명령 | `--password*`의 역할 | `--new-pw*`의 역할 |
|------|---------------------|--------------------|
| `drive take-ownership` | — (MSID 자동 사용) | 새 SID 비밀번호 |
| `drive setup` | — (MSID 자동 사용) | 새 SID 비밀번호 (= Admin1 PIN) |
| `drive activate` | SID 비밀번호 | — |
| `user set-pw` | Admin1 비밀번호 | 새 사용자 비밀번호 |
| `user setup`  | Admin1 비밀번호 | 새 사용자 비밀번호 |

플래그 이름은 동일 (`--new-pw / --new-pw-env / --new-pw-file`). 해석 우선순위는 literal → env → file 순.
...
- `drive take-ownership` / `drive activate` / `drive setup` (provisioning)
- `drive revert` (AdminSP 또는 LockingSP)
- `drive psid-revert`
- `range setup` / `range erase`
- `band setup` / `band erase` (Enterprise — BandMaster / EraseMaster 자격증명 필요)
...
- **`drive take-ownership` / `drive setup` 의 입력은 *새* 비밀번호**: 현재 SID(공장 상태는 MSID)는 facade가 자동으로 읽어 인증한다. `--password*`가 아니라 `--new-pw*` 를 사용한다. 이미 ownership을 받은 드라이브에서는 실패 — 다시 take-ownership 하려면 `drive revert --sp admin` 으로 먼저 공장으로 돌려야 한다.
- **`drive activate` 의 인증**: SID 비밀번호로 인증. take-ownership 직후의 SID 또는 사용자 지정 SID. 활성화 후 LockingSP Admin1 PIN은 SID와 동일한 값으로 초기화되므로 곧바로 `range setup`이 가능.
- **`drive setup`은 멱등이 아니다**: 이미 ownership 받은 드라이브에 두 번째로 실행하면 take-ownership 단계에서 `NotAuthorized` (EC_AUTH=4) 반환. 자동화는 먼저 `drive discover --json` 으로 `locking_enabled` 를 보고 분기해야 한다 (아래 "조건부 provisioning" 레시피 참조).
- **`user setup` 은 LockingSP 활성화 이후**: `drive activate` (또는 `drive setup`) 가 끝나기 전에는 LockingSP 가 비활성이므로 `SP_DISABLED` (St=0x05) 로 실패. 사전 조건 확인 후 실행.
...
### 초기 provisioning (공장 드라이브 → Range 구성까지)
```bash
# 1) 공장 드라이브에 ownership + Locking SP 활성화 (한 번에)
NEW_PW="$(cat /secure/admin1.pw)"
cats-cli -d /dev/nvme0 --force drive setup --new-pw "$NEW_PW"

# 2) Range 1 구성 — Admin1 PIN은 SID와 동일하게 초기화되었으므로 같은 pw 사용
cats-cli -d /dev/nvme0 -p "$NEW_PW" --force \
         range setup --id 1 --start 0 --len 1024000

# 3) User1 활성화 + 비밀번호 + Range1 할당 (한 번에)
USER_PW="$(cat /secure/user1.pw)"
cats-cli -d /dev/nvme0 -p "$NEW_PW" \
         user setup --id 1 --range 1 --new-pw "$USER_PW"
```

### 조건부 provisioning (이미 잠금 활성화된 드라이브는 건너뜀)
```bash
# ADMIN_PW 환경변수에 새 SID 비밀번호를 미리 채워둔 상태.
# drive setup 은 *새* 비밀번호를 받으므로 --new-pw-env 사용 (--pw-env 아님).
if cats-cli -d /dev/nvme0 --json drive discover \
     | jq -e '.locking_enabled == false' >/dev/null; then
  cats-cli -d /dev/nvme0 --force drive setup --new-pw-env ADMIN_PW
