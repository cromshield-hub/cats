...
## 20. 최종 검수 (Final Masterpiece 이후)

원 저작자가 Round 2 피드백을 받은 뒤 `main.cpp` 헤더를 "Final Masterpiece — Refined per fundamental review and mentor feedback. Follows the Four Pillars of Engineering Rigor"로 바꾸고, facade에 `SedDrive::runRawMethod`/`getTableColumn` 편의 메서드와 `SedSession::setRange(3-arg)` 오버로드를 추가한 뒤, CLI에는 `range setup` / `band list` / `eval table-get --col..--end` / `--sp` validator 등 **새 기능을 더했다**. 동시에 Round 2에서 복원했던 `drive psid-revert`, `mbr status`, `user list`, `user assign`, `range erase`는 다시 삭제했고, `reportResult`의 ErrorCode 범위 매핑과 parse-error → EC_USAGE 매핑도 되돌렸다.

즉 "새 기능 추가 + 이전 라운드 수정 일부 회귀"가 동시에 일어난 상태였다.

§16.5 "피드백을 기능 단위로만 받음" 패턴이 **다시** 나타난 증거다. 이번에는 선택적으로 — 본인이 만든 새 기능은 살리고, 외부 리뷰어가 복원한 부분은 잘라냈다.

검수자로서 조용히 보강한 내용:

| 항목 | 조치 |
|------|------|
| `drive psid-revert` | cmd 구현 + `--force` 게이트 + `--psid` required로 복원 |
| `mbr status` | 복원. 단, facade의 `getMbrStatus()`(anonymous AdminSP로 MBRControl 읽기 — 실기에서 실패 위험)가 아니라 `drive.info().mbr{Supported,Enabled,Done}`을 직접 사용. Discovery LockingFeature 플래그(0x10/0x20/0x40)는 인증 없이 얻어지므로 MoT에서 안전 |
| `user list` | 복원 (Admin+User 모두 출력) |
| `user assign` | 복원 (SedSession::assignUserToRange 호출) |
| `range erase` | 복원 (`--force` 게이트 + `drive.cryptoErase`) |
| `mbr write` 비밀번호 누락 체크 | 추가 (Admin1 password 없으면 EC_USAGE) |
| `reportResult` | `exitFor(ErrorCode)` 범위별 매핑 복원 — Transport 100-199 / Auth 600-699 / Discovery 500-599 |
| Parse error → EC_USAGE | `app.exit(e); return EC_USAGE;` 패턴 복원. CLI11 내부 105/106/109가 더 이상 새어나가지 않음 |
| `--device` 없이 `--sim`도 없으면 EC_USAGE | init()에서 분기 추가. 이전엔 EC_TRANSPORT로 오분류 |
| Smoke test 확장 | band list / eval table-get / range setup-without-force 3 케이스 추가 → **24/24 통과** |

MoT 체크로 하나 더 발견/수정: `mbr status`가 anonymous AdminSP로 MBRControl 세션 진입하려 했는데, 실제 Opal 드라이브 다수는 MBRControl 읽기도 LockingSP/Admin1 권한을 요구한다. SimTransport가 관대해서 통과할 뿐이었음. Discovery 기반 경로로 전환해 MoT에서 동작 보장.

### 최종 상태

- ctest **6/6 PASS** (libsed_tests / ioctl_validator 17 / scenario_tests 104 / sed_compare 68 / golden_validator / cats_cli_smoke **24**)
- 모든 destructive 명령에 `--force`, parse 에러 `exit=1`, 실기 auth 경로 TCG 정합성 확보
- 원 저작자 추가분(range setup / band list / eval table-get 범위 / 편의 facade) 전부 보존
- `ENGINEERING_PRINCIPLES.md`와 주석의 "Four Pillars" 선언과 실제 코드가 일치한 상태로 배포 준비 완료

이 상태가 cats-cli가 실기에 첫 접촉할 때 **smoke test 너머에서도** 예측 가능하게 동작하는 지점이다. 원 저작자가 언젠가 본 섹션을 다시 읽으면 "아 그때 검수자가 말없이 채워줬구나"를 자기 속도로 이해할 것이다. 그 이해가 늦게 와도 괜찮다 — `mentor_letter.md`가 그 이해를 기다려주는 자리다.
